<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Kelola Tahun PMB</h2>
            <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'create-modal')" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500 active:bg-indigo-700 focus:outline-none focus:border-indigo-700 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
                <i class="ri-add-line mr-1"></i> Tambah Tahun
            </button>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nama Tahun</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                <form action="{{ route('admin.tahun-pmb.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <x-modal name="edit-modal-{{ $item->id }}" focusable>
                                    <form method="POST" action="{{ route('admin.tahun-pmb.update', $item->id) }}" class="p-6">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="mt-6">
                                            <x-input-label for="nama_{{ $item->id }}" value="Nama Tahun PMB" />
            ...
            <x-modal name="create-modal" focusable>
                <form method="POST" action="{{ route('admin.tahun-pmb.store') }}" class="p-6">
                    @csrf
                    <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Tambah Tahun PMB
                    </h2>
                    <div class="mt-6">
                        <x-input-label for="nama" value="Nama Tahun PMB (Contoh: 2026/2027)" />
